...
import { Eye, EyeOff, Check, AlertCircle, RefreshCw, Server, Key, Cpu, Image, MessageSquare, Sliders, Globe, ToggleLeft, ToggleRight, Save } from "lucide-react";
...
interface ConfigForm {
  ...
  // Feature toggles
  enableLogoWatermark: boolean; enableAutoRetry: boolean;
  ...
  return (
    <div className="min-h-screen bg-[#070711] text-white">
      ...
      <main className="max-w-5xl mx-auto px-8 py-8">
        ...
        {/* Error */}
        {error && (
          <div className="mb-6 rounded-xl p-4 bg-red-500/10 border border-red-500/20 text-red-300 text-sm">{error}</div>
        )}
        ...
        {activeTab === "models" && (
          <div className="space-y-8">
            ...
            <Card className="p-8 bg-white/[0.03] border-white/10">
              <h2 className="text-lg font-bold mb-6 flex items-center gap-3">
                <Eye className="w-5 h-5 text-violet-400" /> 视觉理解模型
              </h2>
            ...
            <Card className="p-8 bg-white/[0.03] border-white/10">
              <h2 className="text-lg font-bold mb-6 flex items-center gap-3">
                <MessageSquare className="w-5 h-5 text-violet-400" /> 文案生成模型
              </h2>
              <div className="grid grid-cols-1 gap-2 mb-6">
                {COPY_MODELS.map(m => (
        ...
        {activeTab === "image" && (
          <div className="space-y-8">
            <Card className="p-8 bg-white/[0.03] border-white/10">
              <h2 className="text-lg font-bold mb-6 flex items-center gap-3">
                <Image className="w-5 h-5 text-violet-400" /> 图片生成参数
              </h2>
              <div className="space-y-8">
                <div>
          ...
          <div className="space-y-8">
            <Card className="p-8 bg-white/[0.03] border-white/10">
              <h2 className="text-lg font-bold mb-2 flex items-center gap-3">
                <Eye className="w-5 h-5 text-violet-400" /> 视觉理解提示词模板
              </h2>
              <p className="text-sm text-white/40 mb-4">发送给视觉模型的分析指令，支持多轮对话中的变量占位</p>
              <textarea
                ...
                placeholder="输入视觉分析的 system prompt..."
              />
            </Card>
            <Card className="p-8 bg-white/[0.03] border-white/10">
              <h2 className="text-lg font-bold mb-2 flex items-center gap-3">
                <MessageSquare className="w-5 h-5 text-violet-400" /> 文案生成提示词模板
              </h2>
              <p className="text-sm text-white/40 mb-4">发送给文案模型的核心指令，支持 [受众描述]、[平台类型] 等变量</p>
              <textarea
                value={form.copyPromptTemplate}
                onChange={e => setForm({ ...form, copyPromptTemplate: e.target.value })}
                rows={10}
                className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-white text-sm resize-y"
                placeholder="输入文案生成的 system prompt..."
              />
            </Card>
          </div>
        )}
        ...
        {activeTab === "output" && (
          <div className="space-y-8">
            <Card className="p-8 bg-white/[0.03] border-white/10">
              <h2 className="text-lg font-bold mb-6 flex items-center gap-3">
                <Globe className="w-5 h-5 text-violet-400" /> 输出语言与变体
              </h2>
            ...
            <Card className="p-8 bg-white/[0.03] border-white/10">
              <h2 className="text-lg font-bold mb-6 flex items-center gap-3">
                <Server className="w-5 h-5 text-violet-400" /> 品牌信息
              </h2>
              <div className="grid grid-cols-2 gap-6">
                <div>
        ...
        {activeTab === "features" && (
          <Card className="p-8 bg-white/[0.03] border-white/10">
            <h2 className="text-lg font-bold mb-6 flex items-center gap-3">
              <Sliders className="w-5 h-5 text-violet-400" /> 功能开关
            </h2>
            <div className="space-y-1">
              {[
                ...
                >
                  ...
                  <Toggle
                    enabled={form[item.key as keyof ConfigForm] as boolean}
